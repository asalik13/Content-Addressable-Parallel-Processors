...
For example, if we want to ignore the third character in our search, we would write mask bits [16:23] as high. The circuitry for these are given in figure 1. 
\\\\
...
The circuitry also includes logic for manually setting all bits high and selecting the first tag bit that is on.The circuitry for these are given in figure 1.
\\\\
...
The FSM that acts as a central command center for the CAM is discussed in the next section.